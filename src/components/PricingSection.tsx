    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch max-w-5xl mx-auto">
        ...
        <Card
          ...
          className="flex flex-col justify-between border-[#1A2342] hover:border-[#2D3A63] transition-all relative"
        >
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="font-mono text-xs uppercase tracking-wider text-[#8492A6]">
                {recordings.tag}
              </span>
              {isPromoLive && (
            ...
            <h3 className="font-display text-2xl sm:text-3xl font-bold text-[#F8FAFC] mb-2">
              {recordings.name}
            </h3>
            <p className="text-[15px] text-[#8492A6] mb-6 leading-relaxed">
              {recordings.description}
            </p>

            {/* Price Display */}
            <div className="mb-6 p-4 rounded-xl bg-[#030617] border border-[#1A2342]">
              ...
              <span className="text-[13px] text-[#8492A6] mt-1 block">
            ...
            <div className="space-y-3 mb-8">
              <span className="text-xs uppercase font-mono tracking-wider text-[#8492A6] block mb-2">
                What's Included:
              </span>
              {recordings.features.map((feat, idx) => (
                <div key={idx} className="flex items-start gap-3 text-[15px] text-[#CBD5E1]">
          ...
          <div>
            ...
            {recordings.footnote && (
              <p className="text-[13px] text-[#8492A6] text-center mt-3">
                {recordings.footnote}
              </p>
            )}
            <p className="font-mono text-[11px] text-[#8492A6] text-center mt-2">
        ...
        <Card
          ...
          className="flex flex-col justify-between border-[#CA3A32]/50 shadow-2xl shadow-[#CA3A32]/10 relative bg-gradient-to-b from-[#0A0F29] to-[#0D1438]"
        >
          {/* Top highlight bar */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-[#CA3A32] rounded-t-xl" />

          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-1.5">
                <Sparkles className="w-4 h-4 text-[#CA3A32]" />
                <span className="font-mono text-xs uppercase tracking-wider text-[#F8FAFC] font-semibold">
                  {cohort.tag}
                </span>
              </div>
            ...
            <p className="text-[15px] text-[#CBD5E1] mb-6 leading-relaxed">
              {cohort.description}
            </p>

            {/* Price Display */}
            <div className="mb-6 p-4 rounded-xl bg-[#030617] border border-[#CA3A32]/30">
              <div className="flex items-baseline gap-3">
                {isPromoLive ? (
                  <>
                    <span className="font-mono text-3xl sm:text-4xl font-bold text-[#F8FAFC]">
            ...
            {/* Feature List */}
            <div className="space-y-3 mb-8">
              <span className="text-xs uppercase font-mono tracking-wider text-[#F8FAFC] font-semibold block mb-2">
                Everything in Recordings PLUS:
              </span>
              {cohort.features.map((feat, idx) => (
                <div key={idx} className="flex items-start gap-3 text-[15px] text-[#F8FAFC]">
                  <Check className="w-4 h-4 text-[#CA3A32] shrink-0 mt-1" />
          ...
          <div>
            <Button
              ...
              className="font-bold text-lg shadow-lg shadow-[#CA3A32]/25"
            >
              {cohort.ctaText}
            </Button>
            {cohort.footnote && (
      ...
      <div className="mt-12 flex flex-wrap items-center justify-center gap-6 text-sm text-[#8492A6]">
